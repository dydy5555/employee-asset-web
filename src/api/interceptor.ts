...
const ihttp = axios.create({
  ...
  export async function getSession() {
      try {
        ...
        const res = await fetch(`https://bizweb-adm.kosign.dev/api/v1/session?token=${encodeURIComponent(token!)}&key=${encodeURIComponent(KEY!)}`, { headers });
        const data = await res.json();
        session = data.payload;
        const permission = await fetch(`https://bizweb-adm.kosign.dev/api/v1/admin/permission/${session?.userId}`, { headers });
